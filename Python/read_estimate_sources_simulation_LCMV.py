...
empty_room_cov_sh = mne.compute_raw_covariance(
    filtered_empty_room_data, rank="info", method="shrunk"
)
empty_room_cov_lw = mne.compute_raw_covariance(
    filtered_empty_room_data, rank="info", method="ledoit_wolf"
)
...
empty_room_cov_sh.plot(epochs.info)
empty_room_cov_lw.plot(epochs.info)
...
stc_shrunk = core.gen_lcmv(data_cov_shrunk, empty_room_cov_sh, filters_shrunk)
stc_ledoit_wolf = core.gen_lcmv(
    data_cov_ledoit_wolf, empty_room_cov_lw, filters_ledoit_wolf
)
...
nib.nifti1.save(
    ...
    filename=f"{results_dir}{sub_id}_{band_name}_stat_image_{no_epochs}_ep_{int(freq_source)}Hz",
)
nib.nifti1.save(
    img_fsaverage_shrunk,
    filename=f"{results_dir}{sub_id}_{band_name}_map_image_{no_epochs}_sh_{int(freq_source)}Hz",
)
nib.nifti1.save(
    img_fsaverage_ledoit_wolf,
    filename=f"{results_dir}{sub_id}_{band_name}_fs_image_{no_epochs}_lw_{int(freq_source)}Hz",
)
...
# %% Shrinkage analysis
rescaled_img, coords, z_max = core.rescale_and_find_cut_coords(
    img_fsaverage_shrunk, scale_factor=1
)
plotting.plot_stat_map(
    rescaled_img,
    cut_coords=coords,
